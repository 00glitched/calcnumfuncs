...
## -*- texinfo -*-
## @deftypefn {} {@var{x} =} gausseidelSolve (@var{A}, @var{b},@var{x0},@var{tol},@var{it})
##
## @seealso{}
## @end deftypefn

## Author:  <github@00glitched>
## Created: 2025-05-08

function x = gausseidelSolve (A,b,x0,tol,it)
  # GAUSS-SEIDEL METHOD
        ...
        if length(x0)==length(b)
          ...
          if tol>0
            ...
            if DLUcomp>0
              ...
              DL=A.*(eye(n)+tril(ones(n),-1));
              U =A.*transpose(tril(ones(n),-1));
              while nxix>tol && i<it        # LOOP with tol.cond and max.iter.cond
                x=xi;                       # previous value and iteration number
                xi= inverse(DL)*(b-U*x);    # calc next iteration
